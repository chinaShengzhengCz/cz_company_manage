...
class table_company_zhiya extends discuz_table
{
    public function __construct()
    {

        $this->_table = 'company_zhiya';
        $this->_pk = 'id';
    ...
    public function fetch_page_data($where, $start, $limit, $field = '*', $user_limit = '', $add_where = '', $order = 'id DESC')
    {
        ...
        if ($add_where) {
            ...
            return DB::fetch_all("SELECT {$field} FROM %t WHERE {$where} ORDER BY {$order} LIMIT {$limit} OFFSET {$start}", array($this->_table), $field);
        }
    }
