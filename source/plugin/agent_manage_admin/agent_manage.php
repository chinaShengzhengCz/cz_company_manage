...
if (in_array($_G['groupid'], $view_limit_config)) {
    showmessage($config_view_limit_message, 'plugin.php?id=agent_manage:agent_manage_list_front', array(), array());
}
if (!empty($_GET['code'])) {
    $company_data = C::t('agent_manage_base')->fetch_by_where("code='{$_GET['code']}'");
    if (empty($company_data)) {
        ...
        if (!empty($p_data)) {
            ...
            $p_company = C::t('agent_manage_base')->fetch_all_by_where("code in ('{$p_codes}')");
        }
        ...
        if (empty($_GET['oper'])) {
            ...
            if (!empty($p_coms)) {
                ...
                $codes_scope_tmp = C::t('agent_manage_base')->fetch_all_by_where("code in ('{$p_codes}')", 'code'," and find_in_set('{$top_cate['cate_id']}',cate_map)");
                ...
                foreach ($p_coms as $p_com) {
                    $c_com = DB::fetch_first('SELECT shorter_name,code FROM %t WHERE ' . DB::field('code', $p_com['p_code']), array('agent_manage_base'));
                    $p_str = '';
                    get_parent_company_list($p_com['p_code'], $p_str, $p_code_scope);
                    if ($p_str) {
                        $p_list_str .= trim($p_str, '- ') . " - <a target=\"_blank\" href=\"plugin.php?id=agent_manage_admin:agent_manage_form&oper=view&code={$c_com['code']}\">{$c_com['shorter_name']}</a><br>";
                    } else {
                        $p_list_str .= "<a target=\"_blank\" href=\"plugin.php?id=agent_manage_admin:agent_manage_form&oper=view&code={$c_com['code']}\">{$c_com['shorter_name']}</a><br>";
                    }
                }
            }

            $p_list_str = trim($p_list_str, '- ');
            include template('agent_manage:f_content');
            exit;
            ...
            if (!empty($_GET['type']) && $_GET['type'] == 'data') {
                ...
                if (isset($child_page_config[$_G['groupid']])) {
                    $data = C::t('agent_manage_base')->fetch_page_data($where, $offset, $limit, '*', $child_page_config[$_G['groupid']], $add_where);
                } else {
                    $data = C::t('agent_manage_base')->fetch_page_data($where, $offset, $limit, '*', '', $add_where);
                }

                if (!empty($data)) {
                    $t = C::t('agent_manage_base')->count_by_where($where, $add_where);
                    if (!empty($t['total'])) {
...
if (!empty($_GET['p_code'])) {
    $p_company = C::t('agent_manage_base')->fetch_by_where("code='{$_GET['p_code']}'");
    if (empty($p_company)) {
        exit('数据不存在');
    }
}
...
include template('agent_manage_admin:form');
