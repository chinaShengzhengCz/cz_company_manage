...
class plugin_company_manage_admin
{

    function plugin_company_manage_admin()
    {
        $cate_data = C::t('company_category')->fetch_all();
        if (!empty($cate_data)) {
            $cate_data = gbk2utf8($cate_data);
        }
        include_once template('company_manage_admin:list');
    }
...
function get_parent_company_list($c_code, &$list_str, $p_code_scope)
{
    ...
    if (!empty($p_com)) {
        if (!in_array($p_com['code'], $p_code_scope)) {
            $list_str .= "<a target=\"_blank\" href=\"plugin.php?id=company_manage_admin:company_manage_form&oper=view&code={$p_com['code']}\">{$p_com['shorter_name']}</a>". ' - ' ;
        } else {
            $list_str .= "<a target=\"_blank\" href=\"plugin.php?id=company_manage_admin:company_manage_form&oper=view&code={$p_com['code']}\">{$p_com['shorter_name']}</a>" . ' - ' . get_parent_company_list($p_com['code'], $list_str,$p_code_scope);
        }
    }
    return '';
}
