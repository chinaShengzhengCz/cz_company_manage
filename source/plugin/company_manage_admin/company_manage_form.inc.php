...
if (!empty($_GET['code'])) {
    ...
    if (empty($company_data)) {
        ...
        if (empty($_GET['oper'])) {
            ...
            if (!empty($p_coms)) {
                $codes_scope_tmp = C::t('company_cate_map')->fetch_all_by_where("cate_id={$top_cate['cate_id']}", 'code');
                if (!empty($codes_scope_tmp)) {
                    $p_code_scope = get_values($codes_scope_tmp, 'code');
                } else {
                    exit('分类映射丢失数据丢失 cate_id ' . $top_cate['cate_id']);
                }
                ...
                foreach ($p_coms as $p_com) {
                    ...
                    if ($p_str) {
                        $p_list_str .= trim($p_str, '- ') . " - <a target=\"_blank\" href=\"plugin.php?id=company_manage_admin:company_manage_form&oper=view&code={$c_com['code']}\">{$c_com['shorter_name']}</a><br>";
                    } else {
                        $p_list_str .= "<a target=\"_blank\" href=\"plugin.php?id=company_manage_admin:company_manage_form&oper=view&code={$c_com['code']}\">{$c_com['shorter_name']}</a><br>";
                    }
                }
            }
...
if (!empty($_GET['p_code'])) {
    $p_company = C::t('company_manage_base')->fetch_by_where("code='{$_GET['p_code']}'");
    if (empty($p_company)) {
        exit('数据不存在');
    }
}
...
include template('company_manage_admin:form');
